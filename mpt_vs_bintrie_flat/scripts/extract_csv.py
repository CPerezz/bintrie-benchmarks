...
Modes:
  1. Per-config extraction (default):
     ./extract_csv.py /path/to/results --config bt-gd5 --trie-type bintrie \
       --group-depth 5 --pebble-block-size-kb 4

     ./extract_csv.py /path/to/results --config mpt --trie-type mpt \
       --group-depth 0 --pebble-block-size-kb 4
...
Produces:
  Per-config: erc20_balanceof.csv, erc20_approve.csv, mixed_sload_sstore.csv,
              cache_validation.csv, all_benchmarks.csv
  Consolidated: mpt_vs_bintrie_consolidated.csv
"""
...
NUM_RUNS = int(__import__('os').environ.get('NUM_RUNS', '10'))
...
def consolidate(args):
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / "mpt_vs_bintrie_consolidated.csv"
...
def main():
    ...
    parser.add_argument("--config", help="Config ID (e.g., bt-gd5, mpt)")
    parser.add_argument(
