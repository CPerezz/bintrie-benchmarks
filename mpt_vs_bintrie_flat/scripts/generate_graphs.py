...
def g07_per_slot_total_time(all_data: dict, theme: Theme,
    ...
    n_cfg = len(CONFIGS)
    bar_width = 0.27 if n_cfg > 2 else 0.32
    ...
    for bench_idx, bench in enumerate(active_benches):
        ...
        for cfg_idx, cfg in enumerate(CONFIGS):
            ...
            x = bench_idx + (cfg_idx - (n_cfg - 1) / 2) * bar_width
    ...
    # Ratio annotations: each non-MPT config / MPT
    for bench_idx, bench in enumerate(active_benches):
        m = medians_by_bench.get(bench, {})
        m_mpt = m.get("mpt", 0)
        if m_mpt <= 0:
            continue
        # Stack ratio annotations vertically when multiple non-MPT configs exist
        max_med = max(m.values()) if m else 0
        ratio_lines = []
        for other_cfg in [c for c in CONFIGS if c != "mpt"]:
            m_bt = m.get(other_cfg, 0)
            if m_bt > 0:
                ratio = m_bt / m_mpt
                ratio_lines.append(f"{cfg_label(other_cfg)}/MPT: {ratio:.1f}\u00d7")
        if ratio_lines:
            y_top = max_med * 0.96
            ax.text(bench_idx, y_top, "\n".join(ratio_lines),
