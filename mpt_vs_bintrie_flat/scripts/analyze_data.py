...
def main():
    ...
    print()
    print_separator()
    print("  MANN-WHITNEY U: MPT vs each non-MPT config")
    print_separator()
        ...
        for bench in BENCHMARKS:
            ...
            for other_cfg in [c for c in CONFIGS if c != "mpt"]:
                ...
                if not rows_mpt or not rows_bt:
                    print(f"  [{other_cfg} vs mpt] Insufficient data")
                    continue

                results["mann_whitney"][bench][other_cfg] = {}
                print(f"  [{other_cfg} vs mpt]")
                for metric in COMPARISON_METRICS:
                    ...
                    print(f"    {metric:>22}: MPT {med_mpt:>10.2f}  "
                          f"{other_cfg} {med_bt:>10.2f}"
                          f"  U={stat:.0f}  p={p:.2e} {sig}")
                    results["mann_whitney"][bench][other_cfg][metric] = {
                        "U": round(stat, 2),
    ...
    print()
    print_separator()
    print("  BOOTSTRAP RATIO CIs: median(other) / median(MPT), 95% CI")
    print_separator()
    ...
    for bench in BENCHMARKS:
        ...
        for other_cfg in [c for c in CONFIGS if c != "mpt"]:
            ...
            results["bootstrap_ratios"][bench][other_cfg] = {}
            print(f"  [{other_cfg} vs mpt]")
            for metric in COMPARISON_METRICS:
                if not should_include_metric(bench, metric):
                    continue
    ...
    if HAS_SCIPY:
        for bench in BENCHMARKS:
            ...
            for other_cfg in [c for c in CONFIGS if c != "mpt"]:
                ...
                results["welch_t"][bench][other_cfg] = {}
                print(f"  [{other_cfg} vs mpt]")
                for metric in COMPARISON_METRICS:
                    if not should_include_metric(bench, metric):
                        continue
                    ...
                    if len(run_meds_mpt) < 2 or len(run_meds_bt) < 2:
                        print(f"    {metric:>22}: insufficient runs "
                              f"(MPT={len(run_meds_mpt)}, "
                              f"{other_cfg}={len(run_meds_bt)})")
                        continue

                    t_stat, p = ttest_ind(run_meds_mpt, run_meds_bt,
                                          equal_var=False)
                    sig = significance_stars(p)
                    print(f"    {metric:>22}: t={t_stat:>8.3f}  "
                          f"p={p:.2e} {sig}"
                          f"  (n_mpt={len(run_meds_mpt)}, "
                          f"n_{other_cfg}={len(run_meds_bt)})")
                    results["welch_t"][bench][other_cfg][metric] = {
